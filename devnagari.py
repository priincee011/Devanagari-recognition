...
if canvas_result.image_data is not None:
    ...
    if st.button('Predict'):
        ...
        if model_choice == "character model":
            ...
            prediction_result = f"Predicted character: {nepali_dict[np.argmax(char)]}"
            st.write("Predicted characters are:")
            for i in np.argsort(char[0])[-5:][::-1]:
                st.write(f"{i} ({nepali_dict[i]})", "%.3f" % (char[0][i] * 100), "%")

        elif model_choice == "Neural Network":
            modelsel = keras.models.load_model("digits.h5")
            char = modelsel.predict(img_array.reshape(1, 1024))
            prediction_result = f"Predicted digit: {np.argmax(char)} ({devanagari_digits[np.argmax(char)]})"
            st.write("Predicted digits are:")
            for i in np.argsort(char[0])[-5:][::-1]:
                st.write(f"{i} ({devanagari_digits[i]})", "%.3f" % (char[0][i] * 100), "%")
            ...
            img_array = img_array.reshape(1, -1)
            img_array_pca = pca.transform(img_array)
            proba = logistic.predict_proba(img_array_pca)
            pred = logistic.predict(img_array_pca)
            prediction_result = f"Predicted digit: {pred[0]} ({devanagari_digits[int(pred[0])]})"
            st.write("Predicted digits are:")
            for i in np.argsort(proba[0])[-5:][::-1]:
                st.write(f"{i} ({devanagari_digits[i]})", "%.3f" % (proba[0][i] * 100), "%")
